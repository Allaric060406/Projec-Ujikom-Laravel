                    ...
                    @foreach ($User as $shw)
                        <tr class="bg-gray-50">
                            ...
                            <td class="p-4 text-sm text-gray-700 whitespace-nowrap">
                                <img src="{{asset('images/'.$shw->imagefile)}}" alt="{{asset('images/'.$shw->imagefile)}}" srcset="" style="max-width: 100px;" class="rounded-xl"> 
                            </td>
                            ...
                            <td>
                                <div x-data="{ showModal: false, isLoaded: false }" x-init="() => { isLoaded = true; }">
                                    ...
                                    <!-- Modal -->
                                    <div x-show="showModal && isLoaded" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed z-10 inset-0 overflow-y-auto" x-cloak>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Set isLoaded menjadi true setelah halaman selesai dimuat
        window.addEventListener('load', function() {
            isLoaded = true;
        });

        // Tambahkan event listener untuk menampilkan modal setelah halaman selesai dimuat
        document.addEventListener('readystatechange', function() {
            if (document.readyState === 'complete') {
                setTimeout(function() {
                    // Setelah 100ms, munculkan modal jika halaman sudah dimuat sepenuhnya
                    isLoaded && (document.querySelector('[x-data]').__x.$data.showModal = true);
                }, 100);
            }
        });
    });
